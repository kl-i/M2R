...
\begin{proof}~(\linkto{prop:paidc:normed_ring}
  ...
  Then from \linkto{prop:padic:decomp}{unique decomposition}, 
  ...
  implies it is a unit by \linkto{prop:padic:local}{$\Z_p$ is a local ring}.
  Since units have valuation $1$, $v_p(x + y) = v_p(p^n) \cdot 1 = n$. 

  $(4)$ $v_p(1) = 0$ since $1$ is a unit. 
\end{proof}

\begin{proof}~(\linkto{prop:padic:top_prop}{
  $p$-adic Integers are Power Series in $p$
})
...
\begin{proof}~(\linkto{
  prop:padic:rat:localization
}{$\Q_p$ as Localizing $\Z_p$ at $p$})
...
\begin{proof}~(
  \linkto{prop:padic:rat:top_prop}{Topological Properties of $\Q_p$}
)
